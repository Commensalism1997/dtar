...
const DPROGTEMPBYTES: &str = "{msg:.7}  [{elapsed_precise:.8}] {wide_bar:.4/8} {bytes}/{total_bytes} {percent}%  [{eta:.7}]";
...
pub fn themed_progressbar_bytes(len: u64) -> indicatif::ProgressBar {
    indicatif::ProgressBar::new(len).with_style(indicatif::ProgressStyle::with_template(DPROGTEMPBYTES).unwrap().progress_chars(DPROGCHARS))
}
